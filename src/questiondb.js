...
$(document).on("click", ".questiongamebtn", function (e) {
  e.preventDefault();
  // var contactBudagi = db.ref("/ContactUs/")
  var myname =  $(".questiongamename").val()
  var comment =  $(".questiongamecomment").val()
  if(($(".questiongamename").val()) && ($(".questiongamecomment").val())){
    pushContactDB(myname, comment)
  }
});
function pushContactDB(myname, comment) {
    db.ref("/questiongame/").push().set({
      myname, comment
    })
    $(".questiongamename").val("")
    $(".questiongamecomment").val("")
}


db.ref("/questiongame/").on("value", function(snap){
